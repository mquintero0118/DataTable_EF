...
export default defineComponent({
  ...
  props: {
    ...
    messages: {
      type: Object,
      default: () => ({
        pagingInfo: 'Mostrando registros del {0} al {1} de un total de {2} registros',
        pageSizeChangeLabel: 'Mostrar ',
        gotoPageLabel: ' registros en la pagina: ',
  ...
  setup(props, { emit, slots }) {
    ...
    const setting = reactive({
      ...
      offset: computed(() => (setting.page - 1) * setting.pageSize + 1),
    ...
    const changePage = (page, prevPage) => {
      ...
      if (!props.isReSearch || page > 1 || page == prevPage) {
        // 非重新查詢發生的頁碼變動才執行呼叫查詢 (Call
        // query will only be executed if the page number is changed without re-query)
        emit('do-search', offset, limit, order, sort);
      }
    };
    ...
    const stringFormat = (template, ...args) => template.replace(/{(\d+)}/g, (match, number) => (typeof args[number] !== 'undefined' ? args[number] : match));

    // Call 「is-finished」 Method
    const callIsFinished = () => {
      if (localTable.value) {
        const localElement = localTable.value.getElementsByClassName(
          'is-rows-el',
        );
